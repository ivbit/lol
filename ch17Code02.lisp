;;;; CHAPTER 17 (Land Of Lisp), PART 2
...
;; "remove-if-not" is deprecated, use "remove-if" instead, with (not (eq ...))
...
(defun describe-objects (loc objs obj-locs)
  (labels
    ...
    (apply
      (function append) ; 1st argument to function "apply"
      (mapcar ; 2nd argument to function "apply"
...
;; (pickup 'whiskey) add (WHISKEY BODY) to the beginning of *object-locations*
(defun pickup (object)
  (cond
    ( ; condition START
    ...
    ( ; default condition START
      t ; constant containing T (everything other than empty list '() is true in lisp)
      '(you cannot get that.) ; if 1st condition returns NIL, return this list
...
) ; defmacro game-action END

(makunbound '*chain-welded*)
(fmakunbound 'weld)
